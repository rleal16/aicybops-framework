"""BaseModel wrapper for modelos_uc AE demo script."""
...
from aicybops_models.modelos_uc import anomalydetection_ae

_UC_AE_IN_MEMORY_CACHE: Dict[str, Dict[str, List[int]]] = {}
_UC_AE_CACHE_LOCK = threading.Lock()


class ModelosUCAEDetector(BaseModel):
    """Unsupervised autoencoder anomaly detector backed by run_pipeline()."""
    ...
    def _run_pipeline(self) -> Dict[str, Any]:
        ...
        key = self._cache_key()
        with _UC_AE_CACHE_LOCK:
            cached = _UC_AE_IN_MEMORY_CACHE.get(key)
            if cached is None:
                cached = self._load_persistent_cache()
            if cached is None:
                raw_outputs = anomalydetection_ae.run_pipeline()
                if not isinstance(raw_outputs, dict):
                    raise TypeError("run_pipeline() must return a dictionary")
                cached = self._to_cache_payload(raw_outputs)
                _UC_AE_IN_MEMORY_CACHE[key] = cached
